...
def _convert_recipient_to_row(recipient):
  ...
  """
  output = [recipient.get(name) for name in
    ['firstname', 'lastname', 'organization', 'address', 'address2', 'address3',
     'city', 'state', 'zipcode']]
...
def _recipient_row_to_xml(row):
  ...
  """
  return ('<address>\n  ' +
          '\n  '.join('<{key}>{value}</{key}>'.format(key=key, value=value or '')
...
def _recipient_list_to_xml(recipients, address_list_name, address_mapping_id=2):
  ...
  """
  addresses = '\n'.join(_recipient_row_to_xml(_convert_recipient_to_row(recipient))
  ...
  return """
    ...
    </addressList>""".format(addresses=addresses, name=address_list_name,
...
def _raise_errors(response, extra_text='', allowed_status=()):
  ...
  """
  if not response.ok:
    raise ValueError("Something went wrong {}: {}".format(extra_text, response.content))
...
class ReturnAddress:
  ...
  """
  def __init__(self, name=None, organization=None, address=None,
...
class Click2MailClient:
  ...
  def post_document(self, document_pdf, document_name=None,
                    document_class='Letter 8.5 x 11', document_format='PDF'):
  ...
  def create_job_from_template(self, template_name):
    ...
    """
    response = self._post('jobs', 'jobTemplate', query={'templateName': template_name})
  ...
  def create_job(self, document_id, address_list_id, document_class='Letter 8.5 x 11'):
    data = {
      'documentClass': document_class,
  ...
  def submit_job(self, job_id, billing_type='Invoice'):
    ...
    """
    return self._post('jobs', str(job_id), 'submit', data={'billingType': billing_type})
