...
class ApiHandler:
  ...
  def get_sites(self, filter_historical=True):
    ...
    r = self._get('Security.svc', 'GetSSOSites/', self.auth_token, self.enterprise_id)
    self._sites = _list_to_dict(r.json(), 'Key', 'Value')
    if filter_historical:
      self._sites = {k: v for k, v in self._sites.items() if not v.startswith('Historical')}
    return self._sites

  def login_site(self, site_id):
    r = self._get('Security.svc', 'SSOSiteLogin/',
                  site_id, self.enterprise_id, self.auth_token, self.timezone_offset)
    ...
    if not program_id:
      raise ValueError(f'Site {self.site_id} (self.site_name) has no SUSO program')

    r = self._post_site('Security.svc', 'UpdateCurrentProgram/', json={'ProgramID': str(program_id)})
  ...
  def get_participants(self, start_date, end_date):
    ...
    for clid in output.CLID.unique():
      r = self._get_site('Actor.svc', 'participant', clid, query={'MaskSSN': 'True'})
      j = r.json()
    ...
    demo_df = pd.DataFrame.from_records([], columns=['CLID', 'guardian_firstname',
      'guardian_lastname', 'SubjectID', 'address', 'zipcode'])
    ...
    for subject_id in output.SubjectID.dropna().astype(int).unique():
      r = self._get_site('TouchPoint.svc', 'ListTouchPointResponses', query={'SubjectID': str(subject_id), 'TouchpointID': '68'})
      j = r.json() or []
      for elt in j:
    ...
    subject_df = pd.DataFrame.from_records([], columns=['SubjectID', 'AuditStaffID', 'referral_date',
      'referral_source', 'grade', 'unexcused_absences', 'is_high_risk', 'school_name',
      'current_referral_status', 'youth_club1', 'youth_club2'])
    ...
    staff_df = pd.DataFrame.from_records([], columns=['AuditStaffID',
      'staff_last_name', 'staff_first_name', 'staff_email'])
