...
from suso import click2mail, eto, render
from suso import database as db
from suso import email
...
class Submitter:
  ...
  def post(self):
    ...
    self.document_id = self.client.post_document(f'{self.pdf_directory}/{self.key}.pdf')
    ...
    self.client._post('jobs', self.job_id, 'update',
                 data={'rtnName': 'Michelle Garcia',
                       'rtnAddress2': '1350 Pennsylvania Avenue NW Suite 533',
                       'rtnAddress1': 'c/o Donald Braman',
                       'rtnZip': '20004', 'rtnCity': 'Washington', 'rtnState': 'DC'})
...
def get_stats_tables(curs):
  ...
  curs.execute("""
    ...
    """, (min_date,))
  month_counts = pd.DataFrame.from_records(curs.fetchall(), columns=['year', 'month', 'count'])
...
def success_email(username, key, curs, num_sent, num_errors):
  client = email.get_client(username, key)
  text = 'There were {} letters sent at this time and there were {} errors\n\n{}'.format(
...
def create_command(config):
  ...
  curs = conn.cursor()
  curs.execute(f"""
    CREATE TABLE {db.STUDENTS_TABLE} (
  ...
  curs.execute(f"""
    CREATE TABLE {db.RANDOMIZER_TABLE} (
  ...
  """)
  curs.execute(f"""
    CREATE TABLE {db.STATUS_TABLE} (
      id INTEGER IDENTITY(1, 1) PRIMARY KEY,
      student_id INTEGER FOREIGN KEY REFERENCES {db.STUDENTS_TABLE}(id),
      status NVARCHAR(20),
      created_at DATETIME NOT NULL DEFAULT GETDATE()
    )
  """)
  ...
  db.insert_student(curs, 1, 'First', 'Last', 'Address', 'Zip', 'GFirst', 'GLast',
                    'Fake CBO', 'Fake Caseworker', 'Fake School', '2018-01-08', 0)
...
def run_command(config, tex, pdf):
  ...
  start_date = (datetime.strptime(start_date, '%Y-%m-%d') - timedelta(days=2)).strftime('%Y-%m-%d')
  ...
  potential_participants['referral_date'] = potential_participants.ProgramStartDate.apply(eto.convert_date)

  # Filter to new participants
  new_participants = potential_participants[~potential_participants.CLID.isin(all_ids)]
  ...
  bad_data_filter = (new_participants.guardian_firstname.isnull() |
  ...
  for df, is_good_record in [[good_participants, 1], [bad_participants, 0]]:
    for _, row in df.iterrows():
      ...
      db.insert_student(curs, row.CLID, row.FName, row.LName,
                        row.address, row.zipcode, row.guardian_firstname,
                        row.guardian_lastname, row.site_name,
                        caseworker_name,
                        row.school_name, row.referral_date, is_good_record)
  ...
  """)
  df = pd.DataFrame.from_records(curs.fetchall(),
                                 columns=['id', 'firstname', 'lastname', 'address',
                                          'zipcode', 'guardian_firstname', 'guardian_lastname',
                                          'cbo', 'caseworker', 'enrolled_date', 'school',
                                          'is_treatment'])
  curs.close()

  # Pull total number of students randomized so far
  curs = conn.cursor()
  total_seen = curs.execute(f"""SELECT COUNT(*) FROM {db.RANDOMIZER_TABLE}""").fetchall()[0][0]
  curs.close()
  ...
  df.loc[df.caseworker.str.startswith('User') & df.cbo.str.startswith('Far South'),
         'caseworker'] = render.CBOs['Far Southeast'].default_contact
  df['caseworker'] = df.caseworker.map(lambda word: ''.join(
    letter for letter in word if not letter.isdigit()), na_action='ignore')
  ...
  data = {
    row.id: {
      ...
      'address': row.address,
      'zipcode': row.zipcode
    } for _, row in df.iterrows() if row.is_treatment > 0
  }
  ...
  client.set_return_address('Don Braman', 'Office of the City Administrator',
  ...
  click.echo("Done submitting to click2mail; {} submitted and {} errors".format(num_success, num_error))

  curs = conn.cursor()
  success_email(config['mailchimp']['username'], config['mailchimp']['key'],
                curs, num_success, num_error)
  curs.close()
  conn.close()
