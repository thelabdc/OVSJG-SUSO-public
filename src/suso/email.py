...
def send_email(
    ...
    text,
    subject=None,
    from_name="Kevin Wilson Bot",
    reply_to="kevin.wilson@dc.gov",
    list_id=None
):
    """
    ...
    Arguments:
      ...
      list_id (str | None): The id of the mailing list to send the email to (this is
        maintained on mailchimp's website). If None, will try to use the
        MAILCHIMP_DEFAULT_LIST_ID environment variable

    Returns:
      dict[str, str]: Information about the sent email
    """
    subject = subject or name
    list_id = list_id or os.environ.get("MAILCHIMP_DEFAULT_LIST_ID")
